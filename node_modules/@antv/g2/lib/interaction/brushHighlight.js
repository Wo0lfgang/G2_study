...
function brush(root, _a) {
    var { brushed = () => { }, brushended = () => { }, brushcreated = () => { }, extent = bboxOf(root), brushRegion = (x, y, x1, y1, extent) => [x, y, x1, y1], reverse = false, fill = '#777', fillOpacity = '0.3', stroke = '#fff' } = _a, style = __rest(_a, ["brushed", "brushended", "brushcreated", "extent", "brushRegion", "reverse", "fill", "fillOpacity", "stroke"]);
...
function brushHighlight(root, _a) {
    var { elements: elementof, siblings: siblingsof = (root) => [], datum, brushRegion, extent: optionalExtent, reverse, series = false, key = (d) => d, bboxOf = (root) => {
        const { x, y, width, height } = root.style;
        return { x, y, width, height };
    }, state = {} } = _a, rest = __rest(_a, ["elements", "siblings", "datum", "brushRegion", "extent", "reverse", "series", "key", "bboxOf", "state"]);
    ...
    const { width: rootWidth, height: rootHeight, x: ordinalX, y: ordinalY, } = bboxOf(root);
    ...
    return brush(root, Object.assign(Object.assign({}, brushStyle), { extent,
        brushRegion,
        reverse, brushended: series ? seriesBrushend : brushended, brushed: series ? seriesBrushed : brushed }));
}
...
function BrushHighlight(_a) {
    ...
    return (target, viewInstances) => {
        ...
        const brush = brushHighlight(plotArea, Object.assign(Object.assign({ elements: utils_1.selectG2Elements, key: (element) => element.__data__.key, siblings: () => selectSiblingContainers(target, viewInstances, brushKey), datum: (0, utils_1.createDatumof)([
            ...
            ]), brushRegion: (x, y, x1, y1) => [x, y, x1, y1], extent: undefined, state: (0, utils_1.mergeState)([options, ...selectSiblingOptions(target, viewInstances, brushKey)], defaultStates) }, defaultOptions), rest));
