...
const DoubleArea = (0, createElement_1.createElement)((g) => {
    ...
    (0, selection_1.select)(g)
        .maybeAppend('connect-path', () => new g_1.Path({}))
        .style('d', connectPath)
        .call(utils_1.applyStyle, connectStyle);
    (0, selection_1.select)(g)
        .maybeAppend('area-path', () => new g_1.Path({}))
        .style('d', areaPath)
...
const Curve = (options) => {
    const { curve, gradient = false, defined = (d) => !Number.isNaN(d) && d !== undefined && d !== null, connectNulls = false } = options, style = __rest(options, ["curve", "gradient", "defined", "connectNulls"]);
    return (P, value, coordinate, theme) => {
        const { mark, shape, defaultShape } = value;
        const _a = (0, utils_1.getShapeTheme)(theme, mark, shape, defaultShape), { defaultColor } = _a, defaults = __rest(_a, ["defaultColor"]);
        ...
        const fill = gradient && sc ? (0, utils_1.computeGradient)(sc, sx, sy, gradient) : color;
        ...
        if (!(0, coordinate_1.isPolar)(coordinate)) {
            ...
            const areaPath = (points) => {
                ...
                return (0, coordinate_1.isTranspose)(coordinate)
                    ? (0, d3_shape_1.area)()
                        .y((_, idx) => Y1[idx][1])
                        .x1((_, idx) => Y1[idx][0])
                        .x0((_, idx) => Y0[idx][0])
