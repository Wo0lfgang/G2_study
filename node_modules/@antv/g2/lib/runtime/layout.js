...
function computeLayout(components, options) {
    const padding = computePadding(components, options);
    const { paddingLeft, paddingRight, paddingTop, paddingBottom } = padding;
    const { width, height, x = 0, y = 0, inset = 0, insetLeft = inset, insetTop = inset, insetBottom = inset, insetRight = inset, margin = 0, marginLeft = margin, marginBottom = margin, marginTop = margin, marginRight = margin, } = options;
    return Object.assign(Object.assign({}, padding), { width,
        height,
        insetLeft,
        insetTop,
        insetBottom,
        insetRight, innerWidth: width - paddingLeft - paddingRight - marginLeft - marginRight, innerHeight: height - paddingTop - paddingBottom - marginTop - marginBottom, marginLeft,
        marginBottom,
        marginTop,
        marginRight,
        x,
 ...
 */
function computePadding(components, options) {
    const positions = [
        'left',
        'right',
        'top',
    ...
    for (const position of positions) {
        const key = `padding${(0, helper_1.capitalizeFirst)(position)}`;
