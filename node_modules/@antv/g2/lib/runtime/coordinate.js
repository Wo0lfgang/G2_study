...
function createCoordinate(layout, partialOptions, library) {
    ...
    const { innerHeight, innerWidth, insetLeft, insetTop, insetRight, insetBottom, marginLeft, marginTop, } = layout;
    const { coordinates: partialTransform = [] } = partialOptions;
    const transform = inferCoordinate(partialTransform);
    const coordinate = new coord_1.Coordinate(Object.assign(Object.assign({}, layout), { x: insetLeft + marginLeft, y: insetTop + marginTop, width: innerWidth - insetLeft - insetRight, height: innerHeight - insetBottom - insetTop, transformations: transform.map(useCoordinate).flat() }));
    return coordinate;
}
...
function inferCoordinate(coordinates) {
    if (coordinates.find((d) => d.type === 'cartesian'))
        return coordinates;
