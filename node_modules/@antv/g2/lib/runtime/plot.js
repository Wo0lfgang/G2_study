...
function plot(options, selection, library, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        context.emitter.emit(event_1.CHART_LIFE_CIRCLE.BEFORE_PAINT);
        // Plot chart.
        ...
        selection
            ...
            .call(applyTranslate)
            .each(function (view) {
            plotView(view, (0, selection_1.select)(this), transitions, library);
        ...
        const viewInstanceof = (viewContainer) => {
            return Array.from(viewContainer.entries()).map(([view, container]) => ({
                ...
                update: createUpdateView((0, selection_1.select)(container), library, context),
            }));
        };
        ...
        context.emitter.emit(event_1.CHART_LIFE_CIRCLE.AFTER_PAINT);
        // Note!!!
...
function bubbleOptions(options) {
    const { coordinate: viewCoordinate = {}, interaction: viewInteraction = {}, marks } = options, rest = __rest(options, ["coordinate", "interaction", "marks"]);
    ...
    return Object.assign(Object.assign({}, rest), { marks, coordinate: newCoordinate, interaction: newInteraction });
}
...
function initializeMarks(options, library) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const channels of scaleChannels.values()) {
            ...
            const scale = (0, scale_1.inferScale)(name, values, options, coordinates, theme, library);
...
function initializeState(markState, options, library) {
    ...
    const components = (0, component_1.inferComponent)(Array.from(scales), options, library);
    const layout = (0, layout_1.computeLayout)(components, options);
    ...
    (0, layout_1.placeComponents)(components, coordinate, layout);
...
function plotView(view, selection, transitions, library) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const areaStyles = areaKeys.map((d) => (0, helper_1.maybeSubObject)(Object.assign({}, theme, style), d));
        ...
        const styleArea = (selection) => selection
            ...
            .each(function (i) {
            applyStyle((0, selection_1.select)(this), areaStyles[i]);
        });
        let px = 0;
        let py = 0;
        let pw = width;
        ...
        const componentsTransitions = selection
            ...
            .append((options) => (0, component_1.renderComponent)((0, util_1.deepMix)({ animate: componentAnimateOptions }, options), coordinate, theme, library)), (update) => update.transition(function (options) {
            const { preserve = false } = options;
            if (preserve)
                return;
            const newComponent = (0, component_1.renderComponent)((0, util_1.deepMix)({ animate: componentAnimateOptions }, options), coordinate, theme, library);
        ...
        for (const [mark, state] of markState.entries()) {
            ...
            const classNames = [constant_1.ELEMENT_CLASS_NAME];
            ...
            const T = selection
                ...
                update.transition(function (data, index) {
                    ...
                    if (animation === null)
                        (0, helper_1.copyAttributes)(this, node);
                    return animation;
                });
            }), (exit) => {
                return exit
                ...
                const exit = new selection_1.Selection(fromElements, null, this.parentNode);
                exit.transition(transition).remove();
                return transition;
            }), (split) => split
                .transition(function (data) {
                // Append splitted shapes.
                const enter = new selection_1.Selection([], this.__toData__, this.parentNode);
...
function plotLabel(view, selection, transitions, library) {
    ...
    const labelShapes = (0, selection_1.select)(labelLayer)
        ...
        .attr('className', constant_1.LABEL_CLASS_NAME), (update) => update.each(function (d) {
...
function createLabelShapeFunction(mark, state, view, library) {
    ...
    return (options) => {
        ...
        const value = Object.assign(Object.assign({}, style), { text: f(text) });
...
function selectFacetElements(selection, facetClassName, elementClassName) {
...
function createMarkShapeFunction(mark, state, view, library) {
    const [useShape] = (0, library_1.useLibrary)('shape', library);
    const { data: abstractData } = mark;
    ...
    return (data, index) => {
        ...
        const value = Object.assign(Object.assign({}, v), { shape, mark: markType, defaultShape, index });
...
function inferTheme(theme = { type: 'light' }) {
    const { type = 'light' } = theme;
    return Object.assign(Object.assign({}, theme), { type });
}
/**
...
function inferInteraction(view) {
    ...
    return Object.entries((0, util_1.deepMix)(defaults, interaction))
        .filter((d) => !!d[1])
        .map(([key, value]) => (Object.assign({ type: key }, value)));
}
...
function updateLayers(selection, marks) {
    ...
    if (labelLayer)
        return;
    selection.append('g').attr('className', constant_1.LABEL_LAYER_CLASS_NAME);
}
...
function applyClip(selection, clip) {
    ...
    selection.style('clipPath', (data) => {
        ...
        return new g_1.Rect({ style: { x, y, width, height } });
    });
}
