...
export function render(options, context = {}, callback) {
    // Initialize the context if it is not provided.
    const { width = 640, height = 480, on } = options;
    const keyed = inferKeys(options);
    const { canvas = Canvas(width, height), library = createLibrary(), emitter = new EventEmitter(), } = context;
    ...
    canvas.ready
        .then(() => plot(Object.assign(Object.assign({}, keyed), { width, height }), selection, library, context))
        .then(() => {
...
export function renderToMountedElement(options, context = {}, callback) {
    // Initialize the context if it is not provided.
    const { width = 640, height = 480, on } = options;
    const keyed = inferKeys(options);
    const { library = createLibrary(), group = new Group(), emitter = new EventEmitter(), } = context;
    ...
    // Plot the chart and mutate context.
    // Make sure that plot chart after container is ready for every time.
    plot(Object.assign(Object.assign({}, keyed), { width, height }), selection, library, context).then(() => {
        emitter.emit(CHART_LIFE_CIRCLE.AFTER_RENDER);
        callback === null || callback === void 0 ? void 0 : callback();
    });
...
export function destroy(options, context = {}) {
