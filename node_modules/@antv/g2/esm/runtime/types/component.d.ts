...
import { G2Theme, G2ViewInstance, GuideComponentPosition, GuideComponentOrientation, IndexedValue, Layout, Vector2 } from './common';
...
export type G2ComponentNamespaces = 'coordinate' | 'encode' | 'mark' | 'infer' | 'palette' | 'scale' | 'shape' | 'statistic' | 'theme' | 'transform' | 'component' | 'animation' | 'action' | 'interaction' | 'interactor' | 'composition' | 'data' | 'labelTransform';
export type G2Component = EncodeComponent | InferComponent | StatisticComponent | ScaleComponent | CoordinateComponent | PaletteComponent | MarkComponent | ShapeComponent | ThemeComponent | GuideComponentComponent | AnimationComponent | InteractionComponent | CompositionComponent | AdjustComponent | TransformComponent | DataComponent | LabelTransformComponent;
export type G2ComponentValue = Transform | Encode | Infer | Statistic | Scale | CoordinateTransform | Palette | Mark | Shape | Theme | GuideComponent | Animation | Interaction | Composition | Adjust | LabelTransform;
export type G2BaseComponent<R = any, O = Record<string, unknown> | void, P = Record<string, unknown>> = {
    (options?: O): R;
    props?: P;
};
...
export type Shape = (points: Vector2[], value: {
    ...
    index?: number;
    [key: string]: any;
}, coordinate: Coordinate, theme: G2Theme, point2d?: Vector2[][]) => DisplayObject;
export type ShapeProps = {
    defaultEnterAnimation?: string;
    defaultUpdateAnimation?: string;
    defaultExitAnimation?: string;
};
export type ShapeComponent<O = Record<string, unknown>> = G2BaseComponent<Shape, O, ShapeProps>;
export type Theme = G2Theme;
export type ThemeComponent<O = Record<string, unknown>> = G2BaseComponent<Theme, O>;
export type GuideComponent = (scales: Scale[], style: Record<string, any>, coordinate: Coordinate, theme: G2Theme) => DisplayObject;
...
export type Animation = (from: DisplayObject[], to: DisplayObject[], value: Record<string, any>, coordinate: Coordinate, defaults: G2Theme['enter' | 'exit' | 'update']) => GAnimation | GAnimation[];
export type AnimationComponent<O = Record<string, unknown>> = G2BaseComponent<Animation, O>;
...
export type Adjust = (points: Vector2[][], domain: number, layout: Layout) => string[];
export type AdjustComponent<O = Record<string, unknown>> = G2BaseComponent<Adjust, O>;
