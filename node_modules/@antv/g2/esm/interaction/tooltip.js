...
import { selectG2Elements, createXKey, selectPlotArea, mousePosition, selectFacetG2Elements, } from './utils';
...
function showTooltip(root, data, x, y, render, event) {
    ...
    tooltipElement.update(Object.assign({ items,
        title }, (render !== undefined && {
...
function groupNameOf(scale, datum) {
    ...
    if (invertAble(scaleColor))
        return scaleColor.invert(color);
...
function uniqueTitles(titles) {
    const valueTitle = new Map(titles.map((d) => [d instanceof Date ? +d : d, d]));
    return Array.from(valueTitle.values());
}
function groupItems(elements, scale, data = elements.map((d) => d['__data__'])) {
    ...
    const newItems = data.flatMap((datum, i) => {
        ...
        return items.map((_a) => {
            ...
            return (Object.assign(Object.assign({}, item), { color, name: groupNameOf(scale, datum) || name || title }));
        });
    });
    return Object.assign(Object.assign({}, (T.length > 0 && { title: T.join(',') })), { items: newItems });
}
function updateRuleY(root, points, _a) {
    var { height, startX, startY } = _a, rest = __rest(_a, ["height", "startX", "startY"]);
    ...
    const createLine = () => {
        const line = new Line({
            style: Object.assign({ x1,
                x2,
                y1,
                y2, lineWidth: 1, stroke: '#1b1e23', strokeOpacity: 0.5 }, rest),
        });
...
export function seriesTooltip(root, _a) {
    var { elements: elementsof, wait = 50, leading = true, trailing = false, scale, coordinate, crosshairs, item, render, startX = 0, startY = 0 } = _a, rest = __rest(_a, ["elements", "wait", "leading", "trailing", "scale", "coordinate", "crosshairs", "item", "render", "startX", "startY"]);
    ...
    const update = throttle((event) => {
        ...
        if (crosshairs) {
            ...
            updateRuleY(root, points, Object.assign(Object.assign({}, ruleStyle), { height, startX, startY }));
        }
 ...
 */
export function tooltip(root, { elements: elementsof, scale, render, wait = 50, leading = true, trailing = false, groupKey = (d) => d, // group elements by specified key
 }) {
    const elements = elementsof(root);
    ...
    const pointerover = throttle((event) => {
        ...
        const data = group.length === 1 ? singleItem(group[0]) : groupItems(group, scale);
...
export function Tooltip(options) {
    ...
    return (target, viewInstances) => {
        ...
        if (isSeries && !facet) {
            return seriesTooltip(plotArea, Object.assign(Object.assign({}, rest), { elements: selectG2Elements, scale,
        ...
        return tooltip(plotArea, Object.assign(Object.assign({}, rest), { elements: selectG2Elements, scale,
