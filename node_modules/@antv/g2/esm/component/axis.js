 ...
 */
function getTicks(scale, domain, tickCount, defaultTickFormatter, tickFilter, tickMethod, position, coordinate) {
    var _a;
    ...
    const labelFormatter = ((_a = scale.getFormatter) === null || _a === void 0 ? void 0 : _a.call(scale)) || defaultTickFormatter;
    ...
    if (isPolar(coordinate) || isTranspose(coordinate)) {
        const axisTicks = filteredTicks.map((d, i, array) => {
            ...
            return {
                value: isTranspose(coordinate) && ((_b = scale.getTicks) === null || _b === void 0 ? void 0 : _b.call(scale)) ? 1 - tick : tick,
...
function labelTransforms(autoHide, autoRotate) {
    const transforms = [];
    if (autoHide)
        transforms.push({ type: 'hide' });
    if (autoRotate)
...
const ArcAxisComponent = (options) => {
    const { order, size, position, orientation, labelFormatter = (d) => `${d}`, tickFilter, tickCount, tickMethod, title, grid: showGrid = false, important = {}, style } = options, rest = __rest(options, ["order", "size", "position", "orientation", "labelFormatter", "tickFilter", "tickCount", "tickMethod", "title", "grid", "important", "style"]);
    return ([scale], value, coordinate, theme) => {
        const { bbox } = value;
        const { domain } = scale.getOptions();
        const ticks = getTicks(scale, domain, tickCount, labelFormatter, tickFilter, tickMethod, position, coordinate);
        const [innerRadius, outerRadius] = radiusOf(coordinate);
        ...
        return new AxisComponent({
            style: deepMix({}, axisTheme, defaultStyle, Object.assign(Object.assign({ type: 'arc', data: ticks, title: titleContent(title), showGrid }, rest), important)),
        });
    };
};
...
function inferDefaultStyle(scale, coordinate, theme, direction, position, orientation) {
    ...
    if (position === 'bottom') {
        return Object.assign(Object.assign({}, themeStyle), { titlePosition: scale.getTicks ? 'right-bottom' : 'bottom', titleTransformOrigin: 'center', titleTransform: scale.getTicks ? 'translate(-100%, 0)' : undefined });
    }
    if (position === 'center') {
        return Object.assign(Object.assign({}, themeStyle), { labelDirection: direction === 'right' ? 'negative' : 'positive', labelTransform: direction === 'center' ? 'translate(50%, 0)' : '', tickDirection: direction === 'right' ? 'negative' : 'positive', labelSpacing: direction === 'center' ? 0 : 4, titleSpacing: isVertical(orientation) ? 10 : 0, tick: direction === 'center' ? false : undefined });
    }
    return themeStyle;
}
const LinearAxisComponent = (options) => {
    const { direction = 'left', important = {}, labelFormatter = (d) => `${d}`, order, orientation, position, size, tickCount, tickFilter, tickMethod, title, transform, style } = options, userDefinitions = __rest(options, ["direction", "important", "labelFormatter", "order", "orientation", "position", "size", "tickCount", "tickFilter", "tickMethod", "title", "transform", "style"]);
    return ([scale], value, coordinate, theme) => {
        const { bbox } = value;
        const { domain } = scale.getOptions();
        const ticks = getTicks(scale, domain, tickCount, labelFormatter, tickFilter, tickMethod, position, coordinate);
        const defaultStyle = inferDefaultStyle(scale, coordinate, theme, direction, position, orientation);
        const { labelAutoRotate = true, labelAutoHide = false, label: showLabel = true, tick: showTick = true, line = false, grid, } = Object.assign({}, defaultStyle, userDefinitions);
        const showGrid = inferGrid(grid, coordinate, scale);
        const gridLength = inferGridLength(position, coordinate);
        const _a = inferAxisLinearOverrideStyle(position, orientation, bbox, coordinate), { showLine = line } = _a, overrideStyle = __rest(_a, ["showLine"]);
        const axisStyle = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, defaultStyle), style), { type: 'linear', data: ticks, title: titleContent(title), showLabel, labelTransforms: labelTransforms(labelAutoHide, labelAutoRotate), showGrid,
            showTick,
            gridLength }), userDefinitions), { 
            // Always showLine, make title could align the end of axis.
            showLine: true }), (!showLine ? { lineOpacity: 0 } : null)), overrideStyle), important);
        return new AxisComponent({
            className: 'axis',
            style: axisStyle,
        });
