...
import { degToRad, keyframeInterpolate, renderExtDo, scaleToPixel, transition } from '../../../util';
import { CLASS_NAMES } from '../constant';
import { baseDependencies } from './utils';
export var getLineAngle = memoize(function (value, cfg) {
    var _a = __read(cfg.angleRange, 2), startAngle = _a[0], endAngle = _a[1];
    return (endAngle - startAngle) * value + startAngle;
}, function (value, cfg) { return __spreadArray([value], __read(cfg.angleRange), false).join(); });
export var getLineTangentVector = memoize(function (value, cfg) {
    if (cfg.type === 'linear') {
        var _a = __read(cfg.startPos, 2), startX = _a[0], startY = _a[1], _b = __read(cfg.endPos, 2), endX = _b[0], endY = _b[1];
        var _c = __read([endX - startX, endY - startY], 2), dx = _c[0], dy = _c[1];
        return vec2.normalize([0, 0], [dx, dy]);
    }
    var angle = degToRad(getLineAngle(value, cfg));
    return [-Math.sin(angle), Math.cos(angle)];
}, function (value, cfg) {
    var dependencies = baseDependencies(cfg);
    cfg.type === 'arc' && dependencies.push(value);
    return dependencies.join();
});
export function getDirectionVector(value, direction, cfg) {
    var tangentVector = getLineTangentVector(value, cfg);
    return ext.vertical([], tangentVector, direction !== 'positive');
}
export var getLinearValuePos = memoize(function (value, cfg) {
    var _a = __read(cfg.startPos, 2), sx = _a[0], sy = _a[1], _b = __read(cfg.endPos, 2), ex = _b[0], ey = _b[1];
    var _c = __read([ex - sx, ey - sy], 2), dx = _c[0], dy = _c[1];
    return [sx + dx * value, sy + dy * value];
}, function (value, cfg) { return __spreadArray(__spreadArray([value], __read(cfg.startPos), false), __read(cfg.endPos), false).join(); });
export var getArcValuePos = memoize(function (value, cfg) {
    var radius = cfg.radius, _a = __read(cfg.center, 2), cx = _a[0], cy = _a[1];
    var angle = degToRad(getLineAngle(value, cfg));
    return [cx + radius * Math.cos(angle), cy + radius * Math.sin(angle)];
}, function (value, cfg) { return __spreadArray(__spreadArray(__spreadArray([value], __read(cfg.angleRange), false), [cfg.radius], false), __read(cfg.center), false).join(); });
export function getValuePos(value, cfg) {
    if (cfg.type === 'linear')
        return getLinearValuePos(value, cfg);
    return getArcValuePos(value, cfg);
}
export function isAxisHorizontal(cfg) {
    return getLineTangentVector(0, cfg)[1] === 0;
}
export function isAxisVertical(cfg) {
...
function getArcAttr(arc) {
    ...
    return __spreadArray(__spreadArray(__spreadArray([], __read(angleRange), false), __read(center), false), [radius], false);
}
function renderArc(container, cfg, style, animate) {
    ...
    return container
        ...
        .data([{ path: getArcPath.apply(void 0, __spreadArray(__spreadArray(__spreadArray([], __read(angleRange), false), __read(center), false), [radius], false)) }], function (d, i) { return i; })
        .join(function (enter) {
        return enter
            .append('path')
            .attr('className', CLASS_NAMES.line.name)
            .styles(__assign({ angleRange: angleRange, center: center, radius: radius }, style))
        ...
        return update
            ...
            .transition(function () {
            var _this = this;
            var animation = keyframeInterpolate(this, getArcAttr(this), __spreadArray(__spreadArray(__spreadArray([], __read(angleRange), false), __read(center), false), [radius], false), animate.update);
            if (animation) {
                var layout = function () {
                    var data = get(_this.style, '__keyframe_data__');
...
function renderLinear(container, cfg, style, animate) {
    ...
    var _f = __read([x1 + (x2 - x1) * r1, y1 + (y2 - y1) * r1], 2), x3 = _f[0], y3 = _f[1];
    var _g = __read([x1 + (x2 - x1) * r2, y1 + (y2 - y1) * r2], 2), x4 = _g[0], y4 = _g[1];
...
function renderAxisArrow(container, type, cfg, style) {
    ...
    if (type === 'arc')
        ...
        shapeToAddArrow = container.select(CLASS_NAMES.line.class);
    if (!showArrow || !lineArrow || (cfg.type === 'arc' && isCircle.apply(void 0, __spreadArray([], __read(cfg.angleRange), false)))) {
...
export function renderAxisLine(container, cfg, style, animate) {
    var type = cfg.type;
    var animation;
    if (type === 'linear')
