...
import { BBox, capitalize, deepAssign, getEventPos, ifShow, select, subObject, subObjects, throttle, toPrecision, } from '../../util';
...
var getMinMax = memoize(function (data) {
    return {
...
var Continuous = /** @class */ (function (_super) {
    ...
    function Continuous(config) {
        var _this = _super.call(this, deepAssign({}, CONTINUOUS_DEFAULT_OPTIONS, config)) || this;
    ...
    Continuous.prototype.render = function (attributes, container) {
        ...
        var ribbonGroup = contentGroup.maybeAppendByClassName(CLASS_NAMES.ribbonGroup, 'g');
        /** handle */
    ...
    Continuous.prototype.renderTitle = function (container) {
        ...
        var finalTitleStyle = __assign(__assign({ width: width, height: height }, style), { text: showTitle ? titleText : '' });
    ...
    Object.defineProperty(Continuous.prototype, "ribbonBBox", {
        get: function () {
            ...
            var _c = __read(this.ifHorizontal([availableHeight, availableWidth], [availableWidth, availableHeight]), 2), availableSize = _c[0], availableLength = _c[1];
            ...
            var _e = __read(this.ifHorizontal([ribbonLength, ribbonSize], [ribbonSize, ribbonLength]), 2), width = _e[0], height = _e[1];
            // 需要考虑 handle 的占用空间
            // todo 为了防止因为 handle 文本变化导致的 ribbon 位置变化，handle size 取最大值
            var finalLabelOccupy = ['top', 'left'].includes(labelPosition) ? labelSize : 0;
            var _f = __read(this.ifHorizontal([edgeLength / 2, finalLabelOccupy], [finalLabelOccupy, edgeLength / 2]), 2), x = _f[0], y = _f[1];
    ...
    Continuous.prototype.renderRibbon = function (container) {
        ...
        var style = subObject(this.attributes, 'ribbon');
        ...
        this.ribbon = container
            .maybeAppendByClassName(CLASS_NAMES.ribbon, function () { return new Ribbon({}); })
            .update(__assign({ x: x, y: y, len: len, size: size, type: type, orient: orient, color: color, block: block, partition: data.map(function (d) { return (d.value - min) / (max - min); }), range: this.ribbonRange }, style));
    };
    ...
    Continuous.prototype.renderHandles = function () {
        ...
        var _b = subObject(this.attributes, 'handle'), formatter = _b.formatter, handleStyle = __rest(_b, ["formatter"]);
        var _c = __read(this.selection, 2), min = _c[0], max = _c[1];
        var style = __assign({ orient: orient }, handleStyle);
        var that = this;
        this.handlesGroup
            ...
            .join(function (enter) {
            return enter
                .append(function () { return new Handle({}); })
                .attr('className', function (d) { return "".concat(CLASS_NAMES.handle, " ").concat(_this.getHandleClassName(d.type)); })
                .styles(style)
                .style('labelText', function (d) { return d.value; })
                .each(function (d) {
                var handle = select(this);
                if (d.type === 'start')
    ...
    Object.defineProperty(Continuous.prototype, "handleShape", {
        ...
        get: function () {
            ...
            var _b = __read(this.ifHorizontal([height, width], [width, height]), 2), size = _b[0], len = _b[1];
            return { width: width, height: height, size: size, len: len };
        },
        enumerable: false,
        configurable: true
    ...
    Continuous.prototype.renderIndicator = function (container) {
        var style = subObject(this.attributes, 'indicator');
        this.indicator = container
            .maybeAppendByClassName(CLASS_NAMES.indicator, function () { return new Indicator({ style: style }); })
            .update(style);
    };
    ...
    Continuous.prototype.renderLabel = function (container) {
        var _a = subObject(this.attributes, 'label'), formatter = _a.formatter, filtrate = _a.filtrate, filter = _a.filter, align = _a.align, labelDirection = _a.labelDirection, tickLength = _a.tickLength, _b = _a.showTick, showTick = _b === void 0 ? false : _b, restStyle = __rest(_a, ["formatter", "filtrate", "filter", "align", "labelDirection", "tickLength", "showTick"]);
        var _c = __read(subObjects(restStyle, ['tick']), 2), tickStyle = _c[0], labelStyle = _c[1];
        var style = __assign(__assign(__assign({ type: 'linear', startPos: [0, 0], endPos: [0, 0], data: this.labelData, showLine: false, showGrid: false, showTick: showTick, tickDirection: labelDirection, labelTransform: 'rotate(0)' }, this.labelStyle), Object.fromEntries(Object.entries(tickStyle).map(function (_a) {
            var _b = __read(_a, 2), k = _b[0], v = _b[1];
            return ["tick".concat(capitalize(k)), v];
        }))), Object.fromEntries(Object.entries(labelStyle).map(function (_a) {
            var _b = __read(_a, 2), k = _b[0], v = _b[1];
            return ["label".concat(capitalize(k)), v];
        })));
        var functionStyle = {
        ...
        var finalLabelStyle = __assign(__assign({}, style), functionStyle);
    ...
    Continuous.prototype.showIndicator = function (value, text) {
        ...
        (_a = this.indicator.node()).setLocalPosition.apply(_a, __spreadArray([], __read(pos), false));
    };
