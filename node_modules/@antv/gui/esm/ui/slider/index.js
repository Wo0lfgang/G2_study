...
import { CustomEvent, Text } from '@antv/g';
import { deepMix } from '@antv/util';
import { GUI } from '../../core/gui';
import { getEventPos, ifShow, normalSeriesAttr, superObject, select, subObject, TEXT_INHERITABLE_PROPS, toPrecision, transition, } from '../../util';
...
var Slider = /** @class */ (function (_super) {
    ...
    function Slider(options) {
        var _this = _super.call(this, deepMix({}, Slider.defaultOptions, options)) || this;
        _this.range = [0, 1];
        _this.animation = {
            duration: 100,
            fill: 'both',
    ...
    Object.defineProperty(Slider.prototype, "sparklineShapeCfg", {
        get: function () {
            ...
            if (orient !== 'horizontal')
                return null;
            var _a = subObject(this.attributes, 'sparkline'), padding = _a.padding, sparklineStyle = __rest(_a, ["padding"]);
            var _b = __read(normalSeriesAttr(padding), 4), top = _b[0], right = _b[1], bottom = _b[2], left = _b[3];
            ...
            return __assign(__assign({ x: bkgLW / 2 + left, y: bkgLW / 2 + top }, sparklineStyle), { zIndex: 0, width: width - bkgLW - left - right, height: height - bkgLW - top - bottom });
        },
        enumerable: false,
        configurable: true
    });
    ...
    Object.defineProperty(Slider.prototype, "availableSpace", {
        get: function () {
            ...
            var _a = __read(normalSeriesAttr(padding), 4), top = _a[0], right = _a[1], bottom = _a[2], left = _a[3];
    ...
    Slider.prototype.renderTrack = function (container) {
        ...
        var trackStyle = subObject(this.attributes, 'track');
        this.trackShape = select(container)
            .maybeAppendByClassName('slider-track', 'rect')
            .styles(__assign(__assign(__assign({}, this.shape), trackStyle), { cursor: brushable ? 'crosshair' : 'default' }));
    };
    Slider.prototype.renderSparkline = function (container) {
        var _this = this;
        var orient = this.attributes.orient;
        var sparklineGroup = select(container).maybeAppendByClassName('slider-sparkline-group', 'g');
        ifShow(orient === 'horizontal', sparklineGroup, function (group) {
            var sparklineStyle = _this.sparklineShapeCfg;
            group
                .maybeAppendByClassName('slider-sparkline', function () { return new Sparkline({ style: sparklineStyle }); })
    ...
    Object.defineProperty(Slider.prototype, "selectionCfg", {
        get: function () {
            var selectionStyle = subObject(this.attributes, 'selection');
            return __assign(__assign({}, selectionStyle), this.calcMask());
        },
        enumerable: false,
        configurable: true
    });
    Slider.prototype.renderSelection = function (container) {
        ...
        var createHandle = function (type) {
            ...
            var handle = _this.foregroundGroup
                .maybeAppendByClassName(className, function () { return new Handle({ style: style }); })
                .style('type', type)
                .update(style);
            return handle;
        };
    ...
    Slider.prototype.calcHandleText = function (handleType) {
        ...
        var handleStyle = subObject(this.attributes, 'handle');
        var labelStyle = subObject(handleStyle, 'label');
        ...
        var temp = this.appendChild(new Text({
            style: __assign(__assign(__assign({ visibility: 'hidden' }, TEXT_INHERITABLE_PROPS), labelStyle), { text: formattedText }),
        }));
        // 文字包围盒的宽高
    ...
    Slider.prototype.getHandleLabelShapeCfg = function (handleType) {
        ...
        if (!showLabel)
            return {};
        var labelStyle = subObject(this.attributes, 'handleLabel');
        return __assign(__assign({}, labelStyle), this.calcHandleText(handleType));
    };
    Slider.prototype.getHandleIconShapeCfg = function () {
        ...
        var labelStyle = subObject(this.attributes, 'handleIcon');
        var cursor = this.getOrientVal(['ew-resize', 'ns-resize']);
        var size = this.getHandleSize();
        return __assign(__assign({}, labelStyle), { shape: handleIconShape, orient: orient, cursor: cursor, size: size });
    };
    Slider.prototype.getHandleShapeCfg = function (handleType) {
        ...
        return __assign(__assign(__assign({ zIndex: 3, showLabel: showLabel, visibility: showHandle ? 'visible' : 'hidden' }, superObject(this.getHandleIconShapeCfg(), 'icon')), superObject(__assign(__assign({}, this.getHandleLabelShapeCfg(handleType)), textCfg), 'label')), { x: x, y: y });
    };
    ...
    Slider.defaultOptions = {
        style: __assign(__assign(__assign({ values: [0, 1], trackLength: 200, trackSize: 20, slidable: true, brushable: true, scrollable: true, orient: 'horizontal', trackZIndex: -1, trackFill: '#416180', trackOpacity: 0.05, selectionCursor: 'move', selectionZIndex: 2, selectionFill: '#5B8FF9', selectionFillOpacity: 0.45, sparklinePadding: 1, padding: 0, showHandle: true, showLabel: true, handleSpacing: 2, formatter: function (val) { return val; } }, superObject(HANDLE_DEFAULT_CFG, 'handle')), superObject(HANDLE_ICON_DEFAULT_CFG, 'handleIcon')), superObject(HANDLE_LABEL_DEFAULT_CFG, 'handleLabel')),
    };
    return Slider;
}(GUI));
export { Slider };
