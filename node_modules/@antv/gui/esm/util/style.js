 ...
 */
export var TEXT_INHERITABLE_PROPS = {
    fontSize: '16px',
...
export function getDefaultStyle(style) {
    ...
    var duplicateStyle = clone(style) || {};
    // 移除其他带状态的样式得到默认样式
 ...
 */
// @ts-ignore
export function getStateStyle(style, state, isMerge) {
    ...
    var stateStyle = get(style, state);
    if (isMerge) {
 ...
 * @returns
 */
export function subObject(style, prefix, invert, transform) {
    if (invert === void 0) { invert = false; }
    if (transform === void 0) { transform = true; }
    var internalStyle = transform ? filterTransform(style) : style;
...
export function subObjects(style, prefix) {
 ...
 * @param prefix
 */
export function superObject(style, prefix) {
    var capitalizeFirstLetter = function (str) {
 ...
 */
export function styleSeparator(style, ignoreStyleDict) {
